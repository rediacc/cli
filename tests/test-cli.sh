...
if [ -z "$TOKEN" ]; then
    print_warning "No token provided. Attempting to login..."
    
    # Try to login
    print_status "Logging in with email: $ADMIN_EMAIL"
    ...
    if echo "$LOGIN_OUTPUT" | grep -qi "successfully logged in\|logged in as"; then
        print_status "Login successful: $LOGIN_OUTPUT"
...
# Token is already stored in config, CLI will use it automatically
# Note: Using --token flag prevents automatic token rotation
