...
# Copy Python files
COPY --chown=$REDIACC_LINUX_USER:$REDIACC_LINUX_GROUP *.py ./

# Copy executable scripts and ensure they're executable
COPY --chown=$REDIACC_LINUX_USER:$REDIACC_LINUX_GROUP rediacc* ./
RUN chmod +x rediacc rediacc-cli rediacc-cli-sync rediacc-cli-term go

# Copy shell scripts
COPY --chown=$REDIACC_LINUX_USER:$REDIACC_LINUX_GROUP *.sh ./
RUN chmod +x *.sh

# Copy configuration
COPY --chown=$REDIACC_LINUX_USER:$REDIACC_LINUX_GROUP rediacc-cli.json ./

# Copy tests directory if needed
COPY --chown=$REDIACC_LINUX_USER:$REDIACC_LINUX_GROUP tests/ ./tests/

# Copy dashboard directory
COPY --chown=$REDIACC_LINUX_USER:$REDIACC_LINUX_GROUP cli-dashboard/ ./cli-dashboard/
...
# Default environment variables for the CLI
ENV SYSTEM_HTTP_PORT=7322
ENV REDIACC_API_URL=http://localhost:7322/api
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
    CMD python3 -c "import sys; import rediacc_cli_core; sys.exit(0)" || exit 1
